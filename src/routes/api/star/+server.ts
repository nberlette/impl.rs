...
import { addStarredProject, removeStarredProject } from "$lib/server/github-auth"
...
export const POST: RequestHandler = async ({ request, cookies }) => {
  ...
  try {
    ...
    if (action === "star") {
      success = await starRepoOnGitHub(accessToken, repository_owner, repository_name)
      if (success) {
        await addStarredProject(session.id, projectId)
      }
    } else {
      success = await unstarRepoOnGitHub(accessToken, repository_owner, repository_name)
      if (success) {
