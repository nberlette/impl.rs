...
export async function getReverseDependencies(crateName: string): Promise<number> {
  const result = await fetchCrates<ReverseDependencies>(`/crates/${crateName}/reverse_dependencies`)
  ...
  try {
    ...
    if (!crateInfo) {
      ...
      const matchingCrate = searchResult?.crates.find(
        (c) =>
          c.repository?.includes(project[0].repository_owner) && c.repository?.includes(project[0].repository_name),
      )
