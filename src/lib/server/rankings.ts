...
async function calculateStarVelocity(
  ...
  const daysPassed = daysBetween(
    new Date(oldestSnapshot.recorded_at),
    new Date(newestSnapshot.recorded_at),
  );
...
function calculateActivityScore(project: ProjectMetrics): number {
  ...
  if (project.last_commit_at) {
    const daysSinceCommit = daysBetween(
      new Date(project.last_commit_at),
      new Date(),
    );
  ...
  if (project.last_release_at) {
    const daysSinceRelease = daysBetween(
      new Date(project.last_release_at),
      new Date(),
    );
...
function calculateMaturityScore(project: ProjectMetrics): number {
  ...
  if (project.github_created_at) {
    const ageInDays = daysBetween(
      new Date(project.github_created_at),
      new Date(),
    );
...
async function calculateHotScore(
  ...
  const score = normalizedVelocity * 0.5 + activityScore * 0.3 +
    communityScore * 0.2;
...
async function calculateTrendingScore(
  ...
  const growthRate = project.stars > 0
    ? ((currentVelocity * 30) / project.stars) * 100
    : currentVelocity;

  let acceleration = 0;
  if (previousVelocity > 0) {
    acceleration = ((currentVelocity - previousVelocity) / previousVelocity) *
      100;
  ...
  const score = normalizedGrowth * 0.4 + normalizedAccel * 0.3 +
    releaseActivity * 0.3;
...
async function calculateNewScore(
  ...
  const score = (normalizedTraction * 0.5 + qualityScore * 0.5) *
    (0.5 + agePenalty * 0.5);
...
async function calculateTopScore(
  project: ProjectMetrics,
): Promise<RankingResult> {
  ...
  const score = normalizedStars * 0.6 + maturityScore * 0.2 +
    ecosystemImpact * 0.2;
...
async function saveRanking(
  ...
  await sql`
    ...
    VALUES (${projectId}, ${rankingType}, ${score}, ${position}, ${
