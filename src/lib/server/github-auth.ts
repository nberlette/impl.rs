...
export async function getUserStarredProjects(userId: number): Promise<number[]> {
...
export async function addStarredProject(userId: number, projectId: number): Promise<void> {
  await sql`
    UPDATE users 
...
export async function removeStarredProject(userId: number, projectId: number): Promise<void> {
  await sql`
    UPDATE users 
...
export function verifyUserSession(token: string): { id: number; github_id: number; username: string } | null {
  try {
